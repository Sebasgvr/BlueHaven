...
function setup() {
    ...
    datosPisos = offCtx.getImageData(0, 0, offCanvas.width, offCanvas.height).data;
    anchoPisos = offCanvas.width;

    bufferPiso = pantallaJuego.drawingContext.createImageData(ANCHO_JUEGO, ALTO_JUEGO / 2);
...
function cercaDelAgua() {
    ...
    for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
            const nx = cx + dx;
            const ny = cy + dy;
            if (nx >= 0 && ny >= 0 && nx < MAPA_ANCHO && ny < MAPA_ALTO) {
                if (MAPA[ny][nx] === 3) return true;
            }
        }
    }
    return false;
...
function dibujarSuelo() {
    ...
    for (let filaLocal = 0; filaLocal < altoBuf; filaLocal++) {
        ...
        for (let x = 0; x < ANCHO_JUEGO; x++) {
            ...
            const texX = Math.floor((mundoX - celdaX) * TAMANO_TILE) & (TAMANO_TILE - 1);
            const texY = Math.floor((mundoY - celdaY) * TAMANO_TILE) & (TAMANO_TILE - 1);
...
function dibujarParedes() {
    ...
    for (let x = 0; x < ANCHO_JUEGO; x++) {
        ...
        const distanciaPerp = resultado.distancia * cos(anguloRayo - jugador.angulo);
...
function dibujarFranjaPared(x, arriba, abajo, texturaX, lado) {
    ...
    pantallaJuego.copy(
        texturaPared,
        posU, 0, 1, texturaPared.height,
        x, arriba, 1, abajo - arriba
    );
