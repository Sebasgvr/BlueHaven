...
function setup() {
  ...
  noLoop(); // el juego queda pausado hasta que auth.js confirme el login
}

// auth.js llama a esta función recién cuando el login/registro salió bien
async function iniciarJuego() {
  ...
  loop(); // recién ahora arranca draw()
}

async function cargarDatosUsuario() {
  const res = await fetch("api/catalogo.php");
  if (!res.ok) throw new Error("No se pudo cargar el catálogo de peces");
  catalogoEspecies = await res.json();

  const resInv = await fetch("api/mi_inventario.php");
  if (!resInv.ok) throw new Error("No se pudo cargar tu mochila");
